    ...
    while 1:
        ...
        if matchin:
            ...
            while i<N:
                ...
                nextline = lines[i]
                if rgxout.match(nextline) or rgxin.match(nextline) or nextline.startswith('#'):
                    break
                elif matchcont := re.match(r'\.\.\. (?P<line>.*?)\s*$', nextline):
                    inputline += '\n' + matchcont.group('line')
                else:
...
class MyInteractiveShell(InteractiveShell):
    displayhook_class = Type(MyDisplayHook)
...
class EmbeddedSphinxShell(object):
    ...
    def __init__(self, exec_lines=None):
        ...
        IP = MyInteractiveShell.instance(config=config, profile_dir=profile)
    ...
    def process_input(self, data, input_prompt, lineno):
        ...
        if not is_suppress and not is_semicolon:
            ...
            #
            if processed_output:
                ret.append(processed_output.strip('\n'))
        elif is_semicolon:
    ...
    def process_block(self, block):
        ...
        input_prompt = self.promptin.format(lineno)
        output_prompt = self.promptout.format(lineno)
        image_file = None
        image_directive = None
    ...
    def process_pure_python(self, content):
        ...
        for lineno, line in enumerate(content):
            ...
            if not multiline:
                modified = u"%s %s" % (fmtin.format(ct), line_stripped)
...
def setup(app):
    ...
    app.add_config_value('ipython_rgxin',
                         re.compile(r'In \[(?P<lineno>.*?)]:\s*(?P<line>.*?)\s*$'), 'env')
