...
class AnalyticFLRWDriver(FLRWDriver, ABC):
    ...
    _ereductor: ClassVar[SymbolicEllipticReduction]

    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__()
        cls._ereductor = SymbolicEllipticReduction.get(cls._epoly_degree, cls._epoly_degree)
    ...
    def comoving_distance_dimless_z1z2(self, z1: _TN, z2: _TN) -> _TN:
        return self._ereductor.Ie(0)(
            map(neg, self._epoly_roots),
            self._epoly_degree*(1,),
            (z1, z2)
        ).real / self._epoly_leading**0.5
