...
class CustomDict(dict):
    def __contains__(self, item):
        if issubclass(item, tuple):
            return True
        return super().__contains__(item)

    def __missing__(self, key):
        if issubclass(key, tuple):
            def _custom_unflatten(values, context):
                return key(values)
            self[key] = ret = NodeDef(_tuple_flatten, _custom_unflatten)
            return ret
        raise KeyError(key)


SUPPORTED_NODES: Dict[Type[Any], NodeDef] = CustomDict()
...
def _broadcast_to_and_flatten(pytree: PyTree, spec: TreeSpec) -> Optional[List[Any]]:
    ...
    if isinstance(spec, LeafSpec):
        return None
    node_type = _get_node_type(pytree)
    if node_type != spec.type and not issubclass(spec.type, node_type):
        return None
